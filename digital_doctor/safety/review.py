...
def _revise_reply(draft_reply: str, instruction: str, user_text: str) -> str:
    prompt = (
    ...
    try:
        revised = extract_final(call_model(prompt, json_mode=False)).strip()
    except Exception:
...
def review_final_response(
    ...
    if action == "crisis":
        ...
        revised = _revise_reply(draft_reply, parsed["revision_instruction"], user_text)
